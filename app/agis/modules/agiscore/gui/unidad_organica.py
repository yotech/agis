...
def unidad_menu(unidad_organica_id):
    auth = current.auth
    myconf = current.conf
    T = current.T
    ...
    item = ('',
        (True if request.controller == 'unidad' else False),
        Accion('UO',
               URL('unidad',
                   'index',
                   args=[unidad_organica_id]),
               auth.user != None),
        sub_menu,
        auth.user != None
        )
    sub_menu.append((
        '',
        False,
        Accion(db.unidad_organica(unidad_organica_id).nombre,
               URL('unidad',
                   'index',
                   args=[unidad_organica_id]),
               auth.user != None),
    ...
    sub_menu.append((
        ...
        Accion(T('Departamentos'),
               URL('unidad', 'departamentos', args=[unidad_organica_id]),
    ...
    sub_menu.append((
        ...
        False,
        Accion(T('Carreras'),
               URL('unidad', 'carreras', args=[unidad_organica_id]),
               auth.has_membership(role=myconf.take('roles.admin'))),
        [],
        auth.has_membership(role=myconf.take('roles.admin'))
    ))
    sub_menu.append((
        ...
        False,
        Accion(T('Turmas'),
               URL('unidad','turmas', args=[unidad_organica_id]),
               auth.has_membership(role=myconf.take('roles.admin'))),
        [],
        auth.has_membership(role=myconf.take('roles.admin'))
    ))
